...
func TestQueries(t *testing.T) {
	...
	res := handler(ctx, NewMsgSubmitProposal(testProposal(), sdk.Coins{sdk.NewInt64Coin(sdk.DefaultBondDenom, 1)}, input.addrs[0]), false, 0, 0)
	var proposalID1 uint64
	require.True(t, res.IsOK())
	cdc.MustUnmarshalBinaryLengthPrefixed(res.Data, &proposalID1)

	res = handler(ctx, NewMsgSubmitProposal(testProposal(), sdk.Coins{sdk.NewInt64Coin(sdk.DefaultBondDenom, 10000000)}, input.addrs[0]), false, 0, 0)
	...
	res = handler(ctx, NewMsgSubmitProposal(testProposal(), sdk.Coins{sdk.NewInt64Coin(sdk.DefaultBondDenom, 1)}, input.addrs[1]), false, 0, 0)
	var proposalID3 uint64
	require.True(t, res.IsOK())
	cdc.MustUnmarshalBinaryLengthPrefixed(res.Data, &proposalID3)

	// input.addrs[1] deposits on proposals #2 & #3
	res = handler(ctx, NewMsgDeposit(input.addrs[1], proposalID2, depositParams.MinDeposit), false, 0, 0)
	res = handler(ctx, NewMsgDeposit(input.addrs[1], proposalID3, depositParams.MinDeposit), false, 0, 0)
	...
	require.True(t, handler(ctx, NewMsgVote(input.addrs[0], proposalID2, OptionYes), false, 0, 0).IsOK())
	require.True(t, handler(ctx, NewMsgVote(input.addrs[0], proposalID3, OptionYes), false, 0, 0).IsOK())

	// Addrs[1] votes on proposal #3
	handler(ctx, NewMsgVote(input.addrs[1], proposalID3, OptionYes), false, 0, 0)
